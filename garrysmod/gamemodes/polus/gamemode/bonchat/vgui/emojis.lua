        ...
        function obj:QuerySearch(query, page, callback)
          ...
          if query ~= lastQuery then
            ...
            for k, v in ipairs(names) do
              -- ПОЛЮС-11: `continue` из оригинала заменён на not-обёртку
              -- (движок GMod его понимает, но чистый luac 5.1 — нет)
              if not (not showTones and string.match(v, "_tone%d?")) then
