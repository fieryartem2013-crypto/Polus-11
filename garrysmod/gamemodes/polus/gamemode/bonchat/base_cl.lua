-- BONCHAT × ПОЛЮС-11 (клиентская основа) — MIT © Bonyoze, github.com/Bonyoze/legacy-bonchat
-- Адаптация «ПОЛЮС-11» v4.8.6 «НАВОДКА»: русские инфо-строки, без паники ветки.
...
local sentBranchWarning = true -- ПОЛЮС-11: предупреждение ветки выключено (адаптация проверена на x86-64 и обычной)
...
function BonChat.Say(text, mode)
  if BonChat.lastMsgTime and CurTime() - BonChat.lastMsgTime < BonChat.CVAR.GetMsgCooldown() then
    if not BonChat.sentWaitMsg then
      ...
      BonChat.SendInfoMessage(":i:error: **Эфир занят — подожди " .. wait .. " сек. перед следующим сообщением!**")
...
function BonChat.InitChat()
  ...
  BonChat.SendInfoMessage(":i:tick: **Чат «ПОЛЮС-11 · ЭФИР» загружен** — аварийный возврат к движковому чату: bonchat_enable 0")
...
concommand.Add("bonchat_reload", function()
  ...
  BonChat.SendInfoMessage(":i:cog: **Панель эфира перезагружена**")
...
concommand.Add("bonchat_clear", function()
  BonChat.ClearChat()
  BonChat.SendInfoMessage(":i:bin: **Чат очищен**")
end)
