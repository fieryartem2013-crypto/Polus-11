...
POLUS_BUILD = "4.8.2" -- версия сборки (v4.8.2 «ДОКЛАД»: окно репортов с кнопками принять/тп к себе/закрыть (/репорты), «!»-сообщения уходят в OOC с позывным и видны команде по-честному, заявка грузчику — своё окно ввода (не пропадает), двойной клик в меню моделей надевает, лидерам 1 место + миграция, учёным шприцы, замок консоли только для Главы (p11_cmdlock), рация: R страхует серверный перехват, HUD: текст мороза и задачи в своих рамках)
...
function GM:PlayerCanSeePlayersChat(text, teamOnly, listener, speaker)
    if IsValid(speaker) then
        ...
        if string.StartWith(t, "!") or string.lower(t) == "/меню" then
