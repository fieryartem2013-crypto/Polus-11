...
POLUS_BUILD = "4.6.4" -- версия сборки (v4.6.4 «ДЕЛО НАЙДЕНО»: починено сохранение ника — CharLoad() никогда не вызывался, теперь ник грузится при рестарте и не спрашивает повторно; авто-меню выбора профы при заходе УБРАНО (открыть вручную: !смена / p11_spawnmenu); во время интро НИЧЕГО не всплывает, анкета аккуратно ждёт конца заставки и открывается после неё)
