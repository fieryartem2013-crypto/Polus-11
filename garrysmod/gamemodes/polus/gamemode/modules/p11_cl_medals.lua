-- ============================================================
--  ПОЛЮС-11 — МЕДАЛИ «ПОЧЁТ» (client) v4.19.4
--  Кэш реестра + помощники отрисовки (ТАБ, ники над головами)
--  + окно вручения + тело вкладки МЕДАЛИ в админке.
--  Сама вкладка монтируется из fw_cl_admin через
--  P11FW.MedalsTabBuild(panel, frame) — админка наш блок зовёт
--  в рантайме, порядок загрузки файлов не важен.
-- ============================================================
...
net.Receive("P11_MedalSync", function(len)
    local data = net.ReadData(len)
    if not data then return end
    local ok, tbl = pcall(util.JSONToTable, util.Decompress(data) or "{}")
...
--- могу ли Я вручать (зеркало серверного правила — для видимости кнопок)
...
function P11.MedalAwardMenu(target)
    ...
    -- порядок: заводские ведомственные первыми, потом штабные
...
-- ============ ВКЛАДКА «МЕДАЛИ» В АДМИНКЕ (Developer+, ур.9) ============
-- Зовётся из fw_cl_admin при сборке админ-меню.
-- Лево — ростер бойцов, право — медали выбранного + выдать/снять.
    ...
    local function RefillRoster()
        ...
        for _, pl in ipairs(player.GetAll()) do
            ...
            for _, id in ipairs(ids) do
                local d = P11.Medals.defs[id]
                glyphs[#glyphs + 1] = (d and d.g) or "?"
            end
            local nick = pl:GetNWString("P11_CharName", "")
            if nick == "" then nick = pl:Nick() end
            local line = lv:AddLine(nick, #ids, table.concat(glyphs, " "))
...
print("[POLUS-11] медали «ПОЧЁТ» (client): реестр, надголовье, ТАБ, вручение, вкладка МЕДАЛИ")
