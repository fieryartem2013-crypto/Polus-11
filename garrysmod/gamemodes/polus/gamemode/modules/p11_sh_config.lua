...
POLUS11.Config = {
    ...
    ColdRateMult    = 0.55, -- v4.10.0 «ГАРАЖ» (заявка «быстро мёрзнет»): общий множитель мороза; живьём: p11_coldrate <множ.> (1 = жёстко, 0.5 = мягко)
    ...
    ThingOfferGapMin  = 900,   -- мин. сек между окнами (15 мин) — v4.14.5 «ТИШИНА»: ивент реже (заявка владельца)
    ThingOfferGapMax  = 1800,  -- макс. сек между окнами (30 мин) — v4.14.5 «ТИШИНА»: ивент реже (заявка владельца)
    ...
    MoneyMax        = 1000000, -- v4.17.0 «КОНТРАБАНДА» (заявка): потолок кошелька 100к → 1 000 000₽
    ...
    ChemlightTime   = 300,    -- v4.8.3: сколько горит химсвет (сек) — было 600 «вечно»
    ChemlightCap    = 14,     -- v4.8.3: полка горящих химсветов на карте (старейший гаснет)
    ...
    ThingAutoDevour = true,  -- v4.8.3: убийство когтями само съедает труп и надевает личину жертвы
    ...
    -- ============ v4.8.1: 3D-ЗВУК И РАДИУС РЕЧИ («ЭФИР») ============
    -- ГОЛОС (p11_sv_voice): вплотную громко, на дистанции тише,
    -- за VoiceHearRadius — тишина (Voice3D = объёмное затухание).
    VoiceHearRadius   = 950, -- дальше этого голоса вообще нет
    VoiceFullRadius   = 340, -- внутри — без ощутимого затухания (метка)
    Voice3D           = true, -- объёмный (3D) голос: гаснет с дистанцией
    -- ТЕКСТ (p11_sv_chat): радиусы каналов в юнитах
    SpeechRadius      = 700,  -- обычная речь
    WhisperRadius     = 180,  -- /шепот — только у самого уха
    ShoutRadius       = 1500, -- /крик — почти через станцию
    LoocRadius        = 500,  -- /looc

    -- ============ v4.8.0: ЧЕСТНЫЕ МИНУТЫ ИГРЫ ============
    -- АФК-фриз счётчика времени: без ввода (клавиши/чат/E) дольше
    -- N минут — минуты игры НЕ капают, пока человек не «оживёт».
    -- Плюс v4.8.0 счёт начинается только после первого спавна
    -- (человек в загрузке — не «играет»). 0 = отключить фриз.
    AFKStopMinutes    = 4,

    -- ============ v4.7.3: СПАВН ПРИ СМЕНЕ ДОЛЖНОСТИ ============
    -- true: взявший должность СРАЗУ встаёт на её точку спавна
    -- (профа > фракция); false: переезжает только после смерти.
    JobTeleportOnChange = true,
}
