...
local HELP = {
    ...
    { h = "КЛАВИШИ", t = "F1 — эта справка • F4 — должности • TAB — состав станции • C (удерживать) — ДЕЙСТВИЯ/ЖЕСТЫ • Q — строительство (пропы-«призраки», E — взять/поставить) • /р + текст — рация • !работа — меню должностей • !репорт — написать админам • !ролл — кубик • /menu — админам панель." },
    { h = "ДОКУМЕНТЫ", t = "У каждого в снаряжении есть удостоверение (SWEP «Документы») с уникальным кодом жизни. ЛКМ/R — предъявить ближайшему: он увидит имя, должность и код. Нечто под маской покажет украденное имя — проверка личности только усиливает паранойю." },
    { h = "КТО ЗДЕСЬ КТО?", t = "Рядовой охраны — периметр и коридоры. Офицер — командует: ПРИКАЗ (!приказ текст — баннер всем) и РОЗЫСК (!розыск ник — подозрительный). Повар — кормит и таскает бочки. Лаборант/Вирусолог — берут кровь и гоняют тесты на лабораторном столе. Инженер — генератор, огнемёт и ремонт." },
