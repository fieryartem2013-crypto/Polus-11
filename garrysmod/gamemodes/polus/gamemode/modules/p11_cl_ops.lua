...
--  таймер, прогресс-бары серий удержания (до 5:00), цветные
...
local HOLD_WIN = 300  -- сек серии для досрочной победы (зеркало сервера)
...
local function OpenPick()
    ...
    rw:SetText("победителям 5000₽  ·  проигравшим 1000₽  ·  удержать ВСЕ точки 5:00 — досрочная победа")
end
...
function P11.OpenOps()
    ...
    local txt = "Сейчас тихо. Командование (ранг 4+) может объявить операцию: сброс вахт, " ..
        ...
        "5 минут — досрочная победа. Награды: 5000₽ победителям, 1000₽ остальным."
    if st.phase == "recruit" then
