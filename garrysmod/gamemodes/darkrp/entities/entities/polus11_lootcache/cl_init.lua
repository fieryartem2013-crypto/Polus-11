...
-- Тайник снабженца: табличка, у пустого — долгий обратный отсчёт.
...
function ENT:Draw()
    ...
    local top = self:GetPos() + Vector(0, 0, 46)
    local ang = Angle(0, (me:EyePos() - top):Angle().y - 90, 90)
    cam.Start3D2D(top, ang, 0.09)
        draw.SimpleTextOutlined("ТАЙНИК СНАБЖЕНЦА", "P11FW.NPC.Big", 0, 0,
            Color(255, 150, 150, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 220))
        if ready then
            draw.SimpleTextOutlined("[ E ] — вскрыть тайник", "P11FW.NPC.Small", 0, 46,
            ...
            draw.SimpleTextOutlined("вычищен · новый выброс через ~" .. left .. " сек", "P11FW.NPC.Small", 0, 46,
