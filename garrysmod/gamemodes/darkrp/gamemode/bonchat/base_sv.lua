-- ============================================================
--  BONCHAT × ПОЛЮС-11 (серверная основа) — MIT © Bonyoze
--  Исходник: github.com/Bonyoze/legacy-bonchat (LICENSE рядом).
--  Адаптация «ПОЛЮС-11» v4.8.6 «НАВОДКА»: фикс nil-текста после
--  PlayerSay (если роутер сборки молчит — сообщение НЕ терялось).
-- ============================================================
...
net.Receive("bonchat_say", function(_, ply)
  ...
  -- ПОЛЮС-11: если роутер вернул nil (команду съели свои модули или
  -- аварийный passthrough) — считаем текст ЖИВЫМ, а не обнуляем
  local routed = hook.Run("PlayerSay", ply, text, teamChat)
  if routed ~= nil then text = routed end
